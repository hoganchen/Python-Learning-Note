...
　　注：C++ 标准（ISO/IEC 14882）【没有】硬性规定 sizeof(bool)==1，但大多数编译器都实现为一字节。
...
def calc_prime_02(max_value):
    prime_list = []
    prime_num = 0
    start_time = time.time()

    if max_value == 2:
        prime_list.append(max_value)
    else:
    ...
    print("total found {} prime number!".format(prime_num))
    # print(prime_list)
    print("Total Elapsed Time: %s seconds\n" % (time.time() - start_time))
    return prime_list


def calc_prime_03(max_value):
    prime_list = []
    prime_num = 0
    start_time = time.time()
...
def calc_prime_05_improve(max_value):
    prime_list = []
    prime_num = 0
    start_time = time.time()
...
def calc_prime_06_interval(min_value, max_value, number=10):
    max_list = []
    prime_list = []
    prime_num = 0
    start_time = time.time()
...
def main():
    ...
    # calc_prime_02(max_value)
    # calc_prime_03(max_value)
    # calc_prime_04(max_value)
    calc_prime_05(max_value)
    # calc_prime_05_improve(max_value)
    calc_prime_06(max_value)
    calc_prime_06_interval(1000000000, 2000000000)
